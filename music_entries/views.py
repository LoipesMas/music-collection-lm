...
def submit(request):
    if request.method == 'POST':
        ...
        form = MusicEntryForm()
        form2 = LinkEntryForm()
        return render(request, 'music_entries/music_submitter.html', {'form': form, 'form2': form2})
...
def submit_link(request):
    if request.method == 'POST':
        ...
        mef = MusicEntryForm()
        form2 = LinkEntryForm()
    return render(request, 'music_entries/music_submitter.html', {'form': mef, 'form2': form2})
...
def view(request, key):
    ...
    if request.GET.get('title'):
        ...
        if not focus_set:
            search_form.fields['title'].widget.attrs.update(
                {'autofocus': 'autofocus'})
            focus_set = True
    if request.GET.get('artist'):
        ...
        if not focus_set:
            search_form.fields['artist'].widget.attrs.update(
                {'autofocus': 'autofocus'})
            focus_set = True
    if request.GET.get('genre'):
        ...
        if not focus_set:
            search_form.fields['genre'].widget.attrs.update(
                {'autofocus': 'autofocus'})
            focus_set = True
    ...
    if not focus_set:
        search_form.fields['title'].widget.attrs.update(
            {'autofocus': 'autofocus'})
        focus_set = True
    return render(request, 'music_entries/music_viewer.html', {'music_entries': mes, 'form': search_form})
...
def edit(request, entry_id):
    ...
    if entry.public_key == key:
        ...
        return render(request, 'music_entries/music_edit.html', {'form': form, 'entry_id': entry_id})
    else:
